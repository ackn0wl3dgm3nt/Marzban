...
def update_user_status(args) -> str:
    user, token, status = args
...
def change_all_users_status(token: str, status: str) -> None:
    ...
    tasks = [(user, token, status) for user in users]

    with mp.Pool(processes=PROCESSES) as pool:
        for result in pool.imap_unordered(update_user_status, tasks):
            logging.info(result)
